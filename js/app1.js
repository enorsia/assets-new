...
/************** Mobile view menu open hide with background ***************/
let lastClickedElement = null;

function handleSubNav(elem) {
    const subMenuItem = elem.parentElement.children[2];
    const subMenuParent = elem.parentElement;

    // If there's a previously clicked element, remove its background
    if (lastClickedElement && lastClickedElement !== subMenuParent) {
        lastClickedElement.style.backgroundColor = "";
        const previousSubMenu = lastClickedElement.children[2];
        if (previousSubMenu) {
            previousSubMenu.classList.remove("d-flex");
            previousSubMenu.classList.add("d-none");
        }
    }
    ...
    if (subMenuItem.className.includes("d-none")) {
        ...
        // Open the clicked sub-menu and change background to white
        subMenuItem.classList.remove("d-none");
        subMenuItem.classList.add("d-flex");
        subMenuParent.style.backgroundColor = "white";

        // Update the last clicked element
        lastClickedElement = subMenuParent;
    } else {
        // Close the sub-menu and reset background
        subMenuItem.classList.remove("d-flex");
        subMenuItem.classList.add("d-none");
        subMenuParent.style.backgroundColor = "";
